...
class OpenAIService:
    """AI service using OpenAI API for chat completions."""
    ...
    def __init__(self):
        ...
        if not self.api_key or self.api_key == 'your-openai-api-key-here':
            logger.warning("OpenAI API key not configured - service will use fallback responses")
            self.api_key = ''
...
- تل مهربان، صبر کوونکی او ګرم اوسئ"""
    ...
    def generate_response(self, message, language='en', rag_context=None):
        """Generate a natural, conversational response using OpenAI API with optional RAG context."""
        try:
            logger.info(f"OpenAI generating response for: {message[:50]}... in {language}")
            
            system_prompt = self.get_system_prompt(language, rag_context)
            
            if not self.api_key or self.api_key == 'your-openai-api-key-here':
                logger.warning("No valid OpenAI API key configured, using fallback response")
